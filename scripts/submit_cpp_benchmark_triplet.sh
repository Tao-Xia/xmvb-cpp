...
dry_run="${DRY_RUN:-0}"
job_prefix="${JOB_PREFIX:-vbbench}"
onnx_model="${ONNX_MODEL:-}"

usage() {
  cat >&2 <<'EOF'
usage: bash scripts/submit_cpp_benchmark_triplet.sh <input.xmi> [xmvb-cpp args...]

This helper submits three single-input benchmark jobs:
  1. standard
  2. adaptive_proposal_all
  3. adaptive_outside_only
...
Environment overrides:
  ...
  DRY_RUN            Default: 0
  JOB_PREFIX         Default: vbbench
  ONNX_MODEL         Default: empty

Examples:
  bash scripts/submit_cpp_benchmark_triplet.sh sample.xmi
  bash scripts/submit_cpp_benchmark_triplet.sh sample.xmi --max-iterations 40
  DUMP_TRACE=1 bash scripts/submit_cpp_benchmark_triplet.sh sample.xmi --max-iterations 40
EOF
}
...
benchmark_dir="${BENCHMARK_DIR:-$benchmark_root/${sample_stem}_cpp_triplet_${timestamp}}"
...
submit_mode "standard" "${job_prefix}_std_${sample_stem}"
submit_mode \
  "adaptive_proposal_all" \
  "${job_prefix}_adpall_${sample_stem}" \
  --structure-space-mode adaptive_mvp \
  --adaptive-determinant-score-mode proposal_all
submit_mode \
  "adaptive_outside_only" \
  "${job_prefix}_adpout_${sample_stem}" \
  --structure-space-mode adaptive_mvp \
