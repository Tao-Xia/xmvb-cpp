...
public:
   ...
   *
   * The reduced TN chart is block-separable. Sparse blocks whiten the
   * accepted-point retraction tangent metric `T^T T`, where `T` is the
   * derivative of `retract_step()` in stored sparse coefficients. Dense
   * full-support blocks remain in the raw candidate chart, where the local Gram
   * matrix `G = D^T D` is applied and solved by analytic block formulas. For
   * those dense blocks we apply the symmetric positive model
   * `M_block = C_block^{1/2} G_block C_block^{1/2}` and return `M_block^{-1} v`.
   * Sparse blocks that cannot be factorized fall back to the diagonal
   * `C_block^{-1}` model.
   */
...
private:
  ...
  struct BlockBasis {
    ...
    // For sparse retraction-whitened blocks, `E^T T^T T E = I` and columns with
    // numerically zero tangent norm are omitted from the reduced chart.
  ...
  void accumulate_block_retract_tangent_combination(
      const BlockBasis& block_basis,
      const Eigen::VectorXd& candidate_coefficients,
      Eigen::VectorXd* packed_vector) const;
