      ...
      if (strstr(str,funcname[i])!=NULL) {
        ...
        *dovbscf = 0;
        for (j=0;j<4;j++) {
          ...
          strncpy(dft,rotyp_str[j],strlen(rotyp_str[j]));
          strncat(dft,funcname[i],strlen(funcname[i]));
...
void parse_pople_basis(char *basis) {
  ...
  if (nstar == 1) 
    ...
    strcat(basname,"_");
    if (comma!=NULL) {
      strncat(basname,ip+1,strlen(ip+1)-strlen(comma));
      strcat(basname,"_");
      strncat(basname,comma+1,strlen(comma+1)-strlen(is));
      strcat(basname,"_");
    }
    else {
      strncat(basname,ip+1,strlen(ip+1)-strlen(is));
      strcat(basname,"_");
    }
  }
...
// read the whole file, delete the blank lines and comments, punch to scr file
  ...
  strncpy(inp_str->basis_name,VBDIR,strlen(VBDIR));
  strncat(inp_str->basis_name,basbuf,strlen(basbuf));
  memset(basbuf,0,sizeof(basbuf));
  strncpy(basbuf,inp_str->aux_name,strlen(inp_str->aux_name));
  strncpy(inp_str->aux_name,VBDIR,strlen(VBDIR));
  strncat(inp_str->aux_name,basbuf,strlen(basbuf));
