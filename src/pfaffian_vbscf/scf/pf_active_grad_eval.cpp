 ...
 * the spatial overlap matrix for one cached Pfaffian state pair.
 *
 * `PfMatrixBuilder` stores closed-shell pair caches with the left spin-orbital
 * matrix already transposed into bra orientation. The generic overlap-metric
 * gradient helper expects the original ket-oriented left pairing matrix and
 * applies the transpose internally, so we transpose `cache.left` back here
 * before calling it.
 */
...
RegularizedOverlapData regularize_overlap(const ConstMatrixRef& overlap_matrix) {
  ...
  if (eigensolver.info() != Eigen::Success) {
    throw std::runtime_error("failed to diagonalize the Pfaffian overlap matrix");
  }
  ...
  if (llt.info() != Eigen::Success) {
    throw std::runtime_error("failed to regularize the Pfaffian overlap matrix");
  }
  return regularized;
}
...
PfActiveGradResult PfActiveGradEval::eval(
  ...
  if (!use_closed_shell_cache_path &&
    ...
    throw std::invalid_argument(
        "Pf active-space gradients currently support closed-shell states and "
  ...
  auto mats = matrix_builder.build(
      basis,
      active_space,
      use_closed_shell_cache_path);
  ...
  const RegularizedOverlapData regularized_overlap =
      regularize_overlap(dense_mat(mats.s, basis.n_states, "pf_overlap_matrix"));
  Matrix overlap_matrix = regularized_overlap.matrix;
  Matrix hamiltonian_matrix =
      symmetrize(dense_mat(mats.h, basis.n_states, "pf_hamiltonian_matrix"));
  ...
  PfActiveGradResult result;
  result.orbital_prepare_dt =
      timed_prepared_active_space.timings.orbital_preparation_wall_time_seconds;
  result.ao_h1e_build_dt =
      timed_prepared_active_space.timings
          .ao_effective_one_electron_wall_time_seconds;
  result.active_h1e_build_dt =
      timed_prepared_active_space.timings.active_one_electron_wall_time_seconds;
  result.active_2e_build_dt =
      timed_prepared_active_space.timings.active_two_electron_wall_time_seconds;
  ...
  const int n_active_orbitals = basis.n_active_orbitals;
  ...
  const Eigen::VectorXd coefficients =
      ground_state_coefficients(
          ...
          basis.n_states);
  const Matrix hamiltonian_gradient =
      coefficients * coefficients.transpose();
  Matrix overlap_gradient =
      -eigen_result.eigenvalues.front() * hamiltonian_gradient;
  if (regularized_overlap.shift > 0.0) {
    overlap_gradient.noalias() -=
        overlap_gradient.trace() *
        (regularized_overlap.min_eigenvector *
         regularized_overlap.min_eigenvector.transpose());
  }
  const std::vector<PairWeight> pair_weights =
      build_pair_weights(hamiltonian_gradient, overlap_gradient);
    ...
    for (long long pair_index = 0; pair_index < n_weighted_pairs; ++pair_index) {
      ...
      try {
        ...
        if (use_closed_shell_cache_path) {
          ...
          const PfKernelCache& cache =
              mats.lower_triangle_pair_caches[weight.cache_index];
...
#pragma omp critical
  ...
  mats.lower_triangle_pair_caches.clear();
  result.scf_result.mats = std::move(mats);
