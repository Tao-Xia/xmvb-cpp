...
PfScfResult PfScfEval::eval(
    const xmvb::vb::CppVbInput& input,
    const PfBasisData& basis,
    double nuclear_repulsion_energy) const {
  const PfPreparedActiveSpaceData prepared_active_space =
...
PfScfResult PfScfEval::eval_active_space(
  ...
  result.mats = matrix_builder_.build(basis, active_space);
  result.matrix_dt = seconds_since(matrix_start_time);

  Matrix overlap_matrix =
      regularize_overlap(dense_mat(result.mats.s, basis.n_states, "result.mats.s"));
