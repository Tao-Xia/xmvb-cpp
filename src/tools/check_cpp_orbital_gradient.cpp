...
int main(int argc, char** argv) {
  try {
    ...
    const auto gradient_result =
        gradient_evaluator.evaluate(load_result.input, load_result.nuclear_repulsion_energy);
    ...
    std::cout << "analytic_gradient_inf_norm = ";
    ...
    for (int report_index = 0; report_index < n_to_report; ++report_index) {
      const int parameter_index = ranked_parameters[static_cast<std::size_t>(report_index)].second;
      xmvb::vb::CppVbInput plus_input = load_result.input;
      xmvb::vb::CppVbInput minus_input = load_result.input;
      plus_input.orbital_preparation_input.orbital_value_table[static_cast<std::size_t>(parameter_index)] +=
          options.step;
      minus_input.orbital_preparation_input.orbital_value_table[static_cast<std::size_t>(parameter_index)] -=
          options.step;

      const double plus_energy =
          evaluate_total_energy(plus_input, options.algorithm, load_result.nuclear_repulsion_energy);
      const double minus_energy =
          evaluate_total_energy(minus_input, options.algorithm, load_result.nuclear_repulsion_energy);
