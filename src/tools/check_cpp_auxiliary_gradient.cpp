...
double evaluate_total_energy_from_auxiliary(
  ...
  const Eigen::Map<const ColumnMajorMatrixXd> basis_overlap_matrix(
      input.orbital_preparation_input.basis_overlap_matrix.data(),
      n_basis_functions,
      n_basis_functions);
  ...
  const auto active_space_one_electron_result =
      active_space_one_electron_builder.build(
          ao_effective_one_electron_matrix,
...
ActiveSpaceMatrices build_active_space_matrices(
  ...
  result.active_orbital_overlap_matrix.assign(
      active_orbital_overlap_matrix.data(),
      active_orbital_overlap_matrix.data() + active_orbital_overlap_matrix.size());
  result.active_one_electron_matrix = active_space_one_electron_result.active_one_electron_matrix;
...
FiniteDifferenceChainBreakdown finite_difference_chain_breakdown(
  ...
  if (plus_matrices.active_one_electron_matrix.size() !=
          minus_matrices.active_one_electron_matrix.size() ||
      plus_matrices.active_one_electron_matrix.size() !=
  ...
  for (std::size_t index = 0;
       index < plus_matrices.active_one_electron_matrix.size();
       ++index) {
    result.one_electron +=
        (plus_matrices.active_one_electron_matrix[index] -
         minus_matrices.active_one_electron_matrix[index]) *
  ...
  try {
    ...
    const auto active_space_matrix_backpropagation_result =
        active_space_matrix_backpropagator.backpropagate(
            ...
            active_space_gradient_result.ao_effective_one_electron_result.ao_effective_one_electron_matrix,
