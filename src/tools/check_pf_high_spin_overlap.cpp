...
void run_case(
  ...
  if (max_overlap_error == nullptr || max_gradient_error == nullptr) {
  ...
  const auto candidate =
      xmvb::pfaffian_vbscf::evaluate_high_spin_open_shell_overlap_and_one_electron(
          left_pair.transpose(),
          right_pair,
          left_blocked_alpha_orbitals,
          right_blocked_alpha_orbitals,
...
int main() {
  try {
    ...
    for (int repeat = 0; repeat < 8; ++repeat) {
      run_case(4, 1, {}, {}, &generator, &distribution, &max_overlap_error, &max_gradient_error);
      run_case(4, 1, {0}, {1}, &generator, &distribution, &max_overlap_error, &max_gradient_error);
      run_case(5, 2, {}, {}, &generator, &distribution, &max_overlap_error, &max_gradient_error);
      run_case(5, 1, {0}, {2}, &generator, &distribution, &max_overlap_error, &max_gradient_error);
    }
    ...
    if (max_overlap_error > tolerance || max_gradient_error > tolerance) {
      std::cerr << "high-spin open-shell overlap validation failed\n";
      return 1;
    }
    return 0;
