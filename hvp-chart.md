...
## 7 Cayley 图表为何不可用：$Q^{\mathrm{T}}Q \neq I$ 导致的度量耦合

### 7.1 理论承诺

Cayley 图表的设计目标是将轨道优化参数化为 6 组切变量 $(A,X,Y,\Delta R_i,\Delta G_a,\Delta L_a)$，其中旋转部分通过 Cayley 变换严格保持 $Q^{\mathrm{T}}SQ=I$，形状部分通过独立变量覆盖活性轨道的非正交自由度。在 §6.2 中已证明图表是单射的（$d_B = n\cdot n_{\mathrm{occ}}$ 精确等于物理轨道系数总数），且在 §6.5 中证明了局部等价性。这些数学性质在纯代数层面是正确的。

### 7.2 数值失败

Cayley 图表在四类体系上以 `XMVB_CPP_USE_CAYLEY_RETRACTION=1` 进行了测试：

| 体系 | 轨道类型 | $n_{\mathrm{basis}}$ (最大块) | Cayley ON 结果 | Cayley OFF ($U_p$) 结果 |
|------|---------|-------------------------------|-------------------|---------------------------|
| FeCl2 | OEO | 289 (全 AO) | 3 步收敛 | 3 步收敛 |
| 241 | HAO | ~40 | 27 步收敛 | 17 步收敛 |
| 240 | HAO | ~30 | **梯度爆炸**（$|g| \to \infty$） | 22 步收敛 |
| MnF2 | HAO | ~30 | **块重建崩溃**（迭代 19） | 201 步收敛 |

HVP 诊断工具也揭示了数值层面的问题。在 F2（HAO）上，以 Cayley ON 运行时：

```
analytic vs FD HVP: max_abs_diff = 0.515, max_rel_diff = 8.6%
（Cayley OFF: max_abs_diff = 2.3e-09, max_rel_diff = 8.1e-10）
```

HVP 的相对误差从 $10^{-9}$（$U_p$ 路径）增加到 $8.6\times 10^{-2}$（Cayley 路径），增大了 **8 个数量级**。更重要的是，这个误差不是均匀的——它集中在少数几个 reduced 分量上（几何拉回修正的 $L_\infty$ 范数是 0.515，而 $L_2$ 范数仅为 0.763，且 total HVP 范数是 ~6）。这说明几何拉回项在某些方向上产生了大幅虚假修正。

### 7.3 根因：$Q^{\mathrm{T}}Q \neq I$

Cayley 图表的核心操作是将物理轨道分解为 $C = QM$，然后在 $Q$ 基中参数化切变量。梯度在 $Q$ 基中的投影为 $g_Q = Q^{\mathrm{T}} g_{\mathrm{raw}}$。

对于 **OEO（全 AO 支撑）**：$S$ 是稠密矩阵但对角优势强，$S \approx I + O(10^{-1})$。因此 $Q^{\mathrm{T}}Q \approx Q^{\mathrm{T}}S Q = I$（因为 $Q^{\mathrm{T}}SQ=I$ 且 $S\approx I$）。这使得 $Q$ 基几乎是一个正交基，切空间中的旋转、梯度投影、以及几何拉回中的 $Q^{\mathrm{T}}Q$ 乘积都近似于单位矩阵。度量耦合可忽略。

对于 **HAO（稀疏 AO 支撑）**：$S$ 在块 AO 支撑上仍然是稠密的，但它的谱结构与 $I$ 差异显著。典型 HAO 块的 $S$ 有特征值范围从 $0.1$ 到 $10$，条件数可达 $10^2$–$10^3$。因此：

$$
Q^{\mathrm{T}}Q \neq I
$$

且 $Q^{\mathrm{T}}Q$ 的偏离程度随 $S$ 的条件数增大而增大。具体来说：
- $Q = T M^{-1/2}$，其中 $M = T^{\mathrm{T}}ST$。即使 $Q^{\mathrm{T}}SQ=I$，$Q^{\mathrm{T}}Q$ 也等于 $M^{-1/2} T^{\mathrm{T}}T M^{-1/2}$，这与 $I$ 的偏离取决于 $T^{\mathrm{T}}T$ 与 $T^{\mathrm{T}}ST$ 之间的差异——即物理轨道在欧几里得度量和 $S$ 度量之间的差异。对 HAO 而言，这一差异很大。

对 Cayley 图表的三层影响：

**层次 1 — 梯度投影错误**。约化梯度 $g_{\mathrm{red}} = J^{\mathrm{T}} g_{\mathrm{raw}}$ 涉及 $Q^{\mathrm{T}} g_{\mathrm{raw}}$ 项（在伴随映射中为 $Q_a^{\mathrm{T}} g_i$、$Q_i^{\mathrm{T}} g_a$ 等）。正确的梯度应使用 $S$-伴随 $Q^{\mathrm{T}}S\, g_{\mathrm{raw}}$，但伴随映射使用的是欧几里得转置 $Q^{\mathrm{T}} g_{\mathrm{raw}}$。两者的差异为 $Q^{\mathrm{T}}(S-I)g_{\mathrm{raw}}$，当 $S$ 远离 $I$ 时不可忽略。

**层次 2 — 几何拉回中的 $Q^{\mathrm{T}}Q$ 错误**。在几何拉回公式（§5.2）中，项 $\operatorname{tr}(g_Q^{\mathrm{T}} \cdot \frac{1}{2}\{\Omega_\alpha,\Omega_v\} \cdot M)$ 的正确计算需要在 $Q$ 基中使用 $S$-加权投影。代码使用 $g_Q = Q^{\mathrm{T}}g_{\mathrm{raw}}$（欧几里得转置）而非 $Q^{\mathrm{T}}S\,g_{\mathrm{raw}}$（$S$-伴随），导致在 $Q^{\mathrm{T}}Q$ 偏离 $I$ 时产生一阶误差。这就是 HVP 诊断中 0.515 误差的来源。

**层次 3 — 回缩后 $Q^{+\mathrm{T}} S Q^+ \neq I$**。Cayley 变换在精确算术中保持 $Q^+ = Q \cdot \mathrm{Cayley}(\Omega)$ 满足 $Q^{+\mathrm{T}} S Q^+ = I$。但在浮点运算中，$\Omega$ 的构建使用了 $Q$ 基中的切变量 $A$、$X$、$Y$，这些变量通过伴随映射 $J^{\mathrm{T}}$ 从梯度投影得到。当 $J^{\mathrm{T}}$ 本身因 $Q^{\mathrm{T}}Q$ 问题而不准确时，$A$、$X$、$Y$ 的值带有误差，导致 $\Omega$ 不是精确的反对称生成元，进而 $Q^{+\mathrm{T}} S Q^+ \neq I$。在下一轮重新正则化中，这一误差被吸收进 $M^+$，但累积的 drift 不会消失，最终导致梯度爆炸（240）或块重建不一致（MnF2）。

### 7.4 为什么 $U_p$ 没有这个问题

逐轨道物理切基 $U_p$ 完全避免了 $Q$ 基分解。每个轨道的切方向 $U_p$ 直接在原始物理系数空间中构建：

$$
\delta c_p = U_p \cdot s_p
$$

梯度投影为 $U_p^{\mathrm{T}} g_p$，回缩为 $c_p^+ = c_p + \delta c_p$ 后接逐列 S-归一化。由于所有操作都在物理空间中，不存在"欧几里得转置 vs $S$-伴随"的歧义——梯度 $g_p$ 就是物理空间中的梯度，切方向 $\delta c_p$ 就是物理空间中的位移。$Q^{\mathrm{T}}Q \neq I$ 的问题根本不会出现。

此外，$U_p$ 的回缩是线性的（$R''(0)=0$），因此不需要几何拉回项。HVP 退化为简单的 $J^{\mathrm{T}} H_y J$，并且 $J$ 和 $J^{\mathrm{T}}$ 使用的是相同的物理空间内积，保证了前向-伴随的一致性。在 F2 上验证的 HVP 误差为 $2.3\times 10^{-9}$，证实了这一点。

### 7.5 对 OEO 而言 Cayley 是否值得

在 OEO 体系（FeCl2）上，Cayley 图表确实工作（3 步收敛），且 HVP 精度为 $1.65\times 10^{-5}$。但这相对于 $U_p$ 路径（HVP 精度 $6.95\times 10^{-5}$，同样的 3 步收敛）的边际改进，**不足以为维护两条代码路径提供理由**：

| 方面 | Cayley (OEO) | $U_p$ (所有体系) |
|------|-------------|---------------------|
| HVP 精度 | $1.65\times 10^{-5}$ | $6.95\times 10^{-5}$ |
| Reduced 维度 (FeCl2) | 4488 | **3855**（更小） |
| 代码复杂度 | 2 条路径 | **1 条路径** |
| HAO 兼容性 | **不可用** | ✓ |
| 几何拉回 | 需要（正确实现约 200 行） | 不需要 |
| 度量假设 | $Q^{\mathrm{T}}Q \approx I$（仅 OEO 满足） | 无 |

$U_p$ 在所有方面都优于或等于 Cayley，且它没有隐藏的度量假设。因此，$U_p$ 是唯一的生产路径。

### 7.6 结论

Cayley 图表的失败不是实现 bug，而是**数学结构层面的不兼容**：S-正交工作框架依赖 $Q^{\mathrm{T}}Q \approx I$ 来保证切映射的准确性，但 HAO 体系的 $S$ 远离 $I$，导致 $Q^{\mathrm{T}}Q$ 与 $I$ 的偏离无法被忽略。所有使用 $Q$ 基的操作——梯度投影、几何拉回、回缩——都会累积这一偏离。

这个问题的本质是：**在 $S \neq I$ 时，欧几里得转置 $Q^{\mathrm{T}}$ 不等于 $S$-伴随 $Q^{\mathrm{T}}S$**。而 Cayley 图表在多个关键位置隐式使用了欧几里得转置，导致当 $S$ 不是 $I$ 的近似时产生不可接受的误差。

修正后的几何拉回公式（§5.2）在理论上是正确的，可以保留作为参考。但它只在 $Q^{\mathrm{T}}Q \approx I$（即 $S \approx I$）的条件下数值可用，这实际上将 Cayley 图表的有效域限制在 OEO 体系——而即使在那里，它相对于 $U_p$ 也没有实质优势。
